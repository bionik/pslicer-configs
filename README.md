...
# Setup 
Open PrusaSlicer once and finish the printer setup with just some random choices to create the config folder.

Go to the config directory, and then:  
`rm -R filament print printer sla_print`  
`git init`  
`git remote add origin git@github.com:bionik/pslicer-configs.git`  
`git fetch`  
